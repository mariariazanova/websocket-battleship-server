...
wsServer.on('connection', (wsClient: WebSocket) => {
  ...
  wsClient.on('close', () => {
    ...
    if (disconnectedUser) {
      const userIndex = wsClients.findIndex(
