...
export const registerResponse = (
  ...
  const existingWsClient = wsClients.find(
    (client) => client.name === wsClient.name,
  );
  ...
  if (existingUser) {
    ...
    } else if (existingUser?.password === loggedUser.password) {
...
export const updateWinnersResponse = (): void => {
  const winnersAndWins = loggedUsers.map((user) => ({
    name: user.name,
    wins: user.wins,
  }));
  ...
  wsClients.forEach((client) => {
    ...
    if (currentUser) {
      ...
      !isUserPlaying &&
        sendResponse(client.id, Command.UPDATE_WINNERS, winnersAndWins);
    }
  });
};
